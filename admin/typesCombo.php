...
$query = 'select c.id as id, i.id as gid, c.heading as h1, c.text as txt, i.src as src from content c join image i on c.img_id = i.id where c.page_id = 2';
...
<tr>
    ...
    <td><?= $r2->src ?></td>
    <td>
    <form action="../views/admin.php?page=typeUD" method="POST">
    <input name="tyID" type="hidden" value="<?= $r2->id ?>"/>
    <input name="tuIID" type="hidden" value="<?= $r2->gid ?>"/>
    <button class="btn btn-primary" type="submit" name="tyD">Delete</button>
    </form>
    </td>
</tr>
<tr>
<form action="../views/admin.php?page=typeUD" method="POST" enctype="multipart/form-data">
    <td><input type="text" name="upTH"></td>
    <td><input type="text" name="upTT"></td>
    <td><input type="file" name="upTI"></td>
    <input name="idT" type="hidden" value="<?= $r2->id ?>"/>
    <input name="idTI" type="hidden" value="<?= $r2->gid ?>"/>
    <td><button class="btn btn-primary" type="submit" name="tyU">Update</button></td>
</form>
</tr>
